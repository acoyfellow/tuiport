<script lang="ts">
  ...
  function sendDemoKey(key: 'left' | 'right') {
    if (demoSocket?.readyState !== WebSocket.OPEN) return;
    demoSocket.send(new TextEncoder().encode(key === 'left' ? '\u001b[D' : '\u001b[C'));
    demoTerminal?.focus();
  }
...
└─────────────────────────────────────┘</pre>
        ...
        <div class="terminal-actions">
          {#if demoStatus === 'live'}
            <button class="demo-control" onclick={() => sendDemoKey('left')} aria-label="Previous panel">←</button>
            <button class="demo-control" onclick={() => sendDemoKey('right')} aria-label="Next panel">→</button>
