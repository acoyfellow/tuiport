    ...
    <section class="hero">
      ...
      <div class:demo-active={demoStatus !== 'idle'} class="terminal" role="group" aria-label="Interactive Tuiport session">
        ...
        <div class="demo-intro" aria-hidden={demoStatus !== 'idle'}>
          ...
          <strong>Control OpenTUI inside a real Cloudflare Container.</strong>
          <p>Your keys travel through a same-origin WebSocket to one constrained application. No shell, commands, filesystem, or credentials.</p>
          <code>browser → Worker → Container → OpenTUI</code>
        </div>
        ...
        <div class="terminal-actions">
          ...
          {:else}
            <button class="start-demo" onclick={startDemo} disabled={demoStatus === 'connecting'}>{demoStatus === 'connecting' ? 'starting…' : demoStatus === 'ended' ? 'start again' : 'try the live demo'}</button>
