<script lang="ts">
  ...
  let demoFrameStarted = false;
  let demoStartupBytes = new Uint8Array();
  ...
  async function startDemo() {
    ...
    demoFrameStarted = false;
    demoStartupBytes = new Uint8Array();
    await tick();
    try {
      ...
      socket.onmessage = async ({ data }) => {
        const bytes =
          typeof data === 'string'
            ...
            : data instanceof Blob
              ...
              : new Uint8Array(data);
        if (demoFrameStarted) {
          demoTerminal?.write(bytes);
          return;
        }

        const combined = new Uint8Array(demoStartupBytes.byteLength + bytes.byteLength);
        combined.set(demoStartupBytes);
        combined.set(bytes, demoStartupBytes.byteLength);
        demoStartupBytes = combined.slice(-32_768);
        const frameMarker = new Uint8Array([0x1b, 0x5b, 0x31, 0x3b, 0x31, 0x48]);
        let markerIndex = -1;
        outer: for (let index = 0; index <= demoStartupBytes.length - frameMarker.length; index++) {
          for (let offset = 0; offset < frameMarker.length; offset++) {
            if (demoStartupBytes[index + offset] !== frameMarker[offset]) continue outer;
          }
          markerIndex = index;
          break;
        }
        if (markerIndex >= 0) {
          ...
          demoTerminal?.write(demoStartupBytes.slice(markerIndex));
          demoStartupBytes = new Uint8Array();
        }
      };
      socket.onerror = () => {
  ...
  function showDemoView(view: 1 | 2 | 3) {
    if (demoSocket?.readyState !== WebSocket.OPEN) return;
    demoSocket.send(new TextEncoder().encode(String(view)));
    demoTerminal?.focus();
  }

  function closeDemo() {
    demoSocket?.send(new TextEncoder().encode('q'));
  }
...
<header>
  <a class="brand" href="/" aria-label="Tuiport home">
    ...
    <span><strong>Tuiport</strong><em>Agent Experience</em></span>
  </a>
    ...
    <section class="hero">
      ...
      <div class="hero-copy">
        ...
        <div class="actions">
          ...
          <a class="secondary" href="/tutorial">Connect a hostname</a>
        </div>
        <div class="hero-meta"><span>Tuiport / Agent Experience</span><span>OpenTUI on Cloudflare</span></div>
      </div>
      <div class="receipt" role="group" aria-label="Install, run, and connect to Tuiport">
        <div class="receipt-head"><span>Install → run → SSH</span><span>local hello world</span></div>
        <pre><span># install</span>
<b>$</b> git clone https://github.com/acoyfellow/tuiport
<b>$</b> cd tuiport && bun install

<span># run the same container image locally</span>
<b>$</b> bun run local

<span># in another terminal</span>
<b>$</b> ssh -p 2222 localhost

╭──────────── tuiport ────────────╮
│ OpenTUI is running over SSH.    │
...
╰─────────────────────────────────╯</pre>
    ...
    <section class="dogfood" aria-labelledby="dogfood-title">
      <div class="dogfood-copy">
        <p class="eyebrow">Live dogfood</p>
        <h2 id="dogfood-title">The same OpenTUI app, running now.</h2>
        <p>This supporting demo skips public SSH ingress and streams one constrained OpenTUI session directly from the deployed Cloudflare Container.</p>
      </div>
      <div class:demo-active={demoStatus !== 'idle'} class="terminal" role="group" aria-label="Interactive Tuiport session">
        <div class="chrome">
          <span class="state"><i></i> {demoStatus === 'live' ? 'live container' : demoStatus === 'connecting' ? 'starting container' : 'interactive demo'}</span>
          <span>{demoStatus === 'live' ? 'real OpenTUI session' : 'no shell · no recording'}</span>
        </div>
        <div class="demo-intro" aria-hidden={demoStatus !== 'idle'}>
          <img src="/images/demo-portal.svg" alt="" />
          <button class="portal-start" onclick={startDemo}>Start live session</button>
        </div>
        ...
        <div class="terminal-actions">
          {#if demoStatus === 'live'}
            <button class="demo-control" onclick={() => showDemoView(1)}>How</button>
            <button class="demo-control" onclick={() => showDemoView(2)}>Safety</button>
            <button class="demo-control" onclick={() => showDemoView(3)}>Runtime</button>
            <button class="demo-control close-demo" onclick={closeDemo}>Close</button>
          {:else if demoStatus !== 'idle'}
            <button class="start-demo" onclick={startDemo} disabled={demoStatus === 'connecting'}>{demoStatus === 'connecting' ? 'starting…' : 'start again'}</button>
          {/if}
        </div>
      </div>
    </section>
...
./tuiport-relay \
  ...
  &#125;
&#125;)</code></pre>
      <h2>Run the complete loop locally</h2><pre><code>git clone https://github.com/acoyfellow/tuiport
cd tuiport &amp;&amp; bun install
bun run local

# in another terminal
ssh -p 2222 localhost</code></pre><p><code>bun run local</code> builds and runs the same container image used by Cloudflare. Docker is required.</p>
...
<style>
  ...
  main{padding-block:58px 96px}.eyebrow{display:flex;align-items:center;gap:.55rem;margin:0 0 28px;color:var(--ax-orange);font:600 .68rem/1 var(--font-mono);letter-spacing:.12em;text-transform:uppercase}.eyebrow::before{content:'';width:1.25rem;height:1px;background:currentColor}.breadcrumb{display:flex;align-items:center;gap:.55rem;margin:0 0 22px;color:var(--ax-muted);font:600 .68rem/1 var(--font-mono);letter-spacing:.08em;text-transform:uppercase}.breadcrumb a{color:var(--ax-orange);text-decoration:none}.breadcrumb a:hover{text-decoration:underline;text-underline-offset:3px}.breadcrumb span:last-child{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.hero{position:relative;display:grid;grid-template-columns:minmax(20rem,.78fr) minmax(30rem,1.22fr);gap:clamp(34px,5vw,72px);align-items:center;min-height:500px;padding:clamp(30px,5vw,68px);border:1px solid var(--ax-border);border-radius:var(--radius-lg);background:linear-gradient(110deg,rgba(8,13,19,.98),rgba(13,31,43,.9));overflow:hidden}.hero-grid{position:absolute;inset:0;background-image:linear-gradient(rgba(255,255,255,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.045) 1px,transparent 1px);background-size:4rem 4rem;mask-image:linear-gradient(90deg,#000,transparent 80%);pointer-events:none}.hero-glow{position:absolute;right:-3%;top:5%;width:min(44rem,57%);aspect-ratio:1;border-radius:50%;background:radial-gradient(circle at 31% 24%,rgba(255,255,255,.2),rgba(113,184,216,.13) 22%,rgba(38,120,164,.12) 47%,rgba(246,130,31,.1) 68%,rgba(53,24,18,.08) 82%,transparent 83%);box-shadow:inset 0 0 42px rgba(255,255,255,.035),0 0 100px rgba(38,120,164,.09);pointer-events:none}.hero-copy,.receipt{position:relative;z-index:1}.badge{display:inline-flex;align-items:center;gap:.5rem;width:max-content;padding:.4rem .55rem;border:1px solid var(--ax-border-strong);border-radius:999px;color:#d7e5ec;background:rgba(8,13,19,.55);backdrop-filter:blur(8px);font:600 .58rem/1 var(--font-mono);text-transform:uppercase;letter-spacing:.055em}.badge i{width:.42rem;height:.42rem;border-radius:50%;background:var(--ax-orange);box-shadow:0 0 0 3px var(--ax-orange-soft)}.prompt-line{display:flex;align-items:center;gap:.65rem;margin:1.75rem 0 .75rem;color:var(--ax-muted);font:500 .68rem/1 var(--font-mono)}.prompt-line span{color:var(--ax-blue-light)}.prompt-line b{color:var(--ax-orange);font-weight:600}h1{max-width:22ch;margin:0 0 1.15rem;font-family:var(--font-mono);font-size:clamp(1.8rem,2.15vw,2.2rem);line-height:1.12;letter-spacing:-.045em;font-weight:600}h1 em{color:var(--ax-blue-light);font-style:normal;font-weight:500}.lede,.summary{max-width:650px;color:#c8d4dd;font-size:clamp(.92rem,1.2vw,1.02rem);line-height:1.68}.actions{display:flex;gap:.65rem;margin-top:2rem}.primary,.secondary{display:inline-flex;align-items:center;justify-content:center;gap:.6rem;min-height:2.7rem;padding:0 1rem;border-radius:var(--radius-md);font-size:.76rem;font-weight:650;text-decoration:none;transition:.15s}.primary{border:1px solid var(--ax-orange);background:var(--ax-orange);color:#1d0b00}.primary:hover{background:#ff9b49}.secondary{border:1px solid var(--ax-border-strong);background:var(--ax-layer-2);color:var(--ax-text)}.secondary:hover{border-color:var(--ax-blue-light)}.hero-meta{display:flex;gap:1.4rem;margin-top:2.2rem;color:var(--ax-muted);font:500 .55rem/1.5 var(--font-mono);text-transform:uppercase;letter-spacing:.07em}.hero-meta span:first-child{color:var(--ax-orange)}
  .receipt{overflow:hidden;border:1px solid var(--ax-border-strong);border-radius:var(--radius-lg);background:#080d13;box-shadow:0 32px 100px rgba(0,0,0,.38)}.receipt-head{display:flex;align-items:center;justify-content:space-between;height:42px;padding:0 .85rem;border-bottom:1px solid var(--ax-border);color:var(--ax-muted);font:600 .58rem/1 var(--font-mono);text-transform:uppercase;letter-spacing:.05em}.receipt-head span:first-child{color:var(--ax-text)}.receipt pre{min-height:360px;margin:0;padding:1.35rem 1.5rem;color:#d7e2ec;font-size:.7rem;line-height:1.55;white-space:pre-wrap}.receipt pre span{color:#708294}.receipt pre b{color:var(--ax-orange)}
  .terminal{position:relative;min-height:368px;text-align:left;padding:0;width:100%;border:1px solid var(--ax-border-strong);border-radius:var(--radius-lg);background:#080d13;color:#d7e2ec;box-shadow:0 32px 100px rgba(0,0,0,.42);overflow:hidden;transform:perspective(900px) rotateY(-2deg)}.chrome{height:42px;display:flex;align-items:center;justify-content:space-between;padding:0 .85rem;border-bottom:1px solid var(--ax-border);color:var(--ax-muted);font:500 .58rem/1 var(--font-mono);text-transform:uppercase;letter-spacing:.05em}.chrome .state{display:flex;align-items:center;gap:.45rem}.chrome i{width:.42rem;height:.42rem;border-radius:50%;background:var(--ax-green);box-shadow:0 0 0 3px var(--ax-green-soft)}.demo-intro{position:relative;min-height:318px;overflow:hidden;background:#0a1118;transition:opacity .15s}.demo-intro::after{content:'';position:absolute;inset:0;background:radial-gradient(circle at 50% 48%,rgba(13,26,36,.12),rgba(5,10,15,.72) 72%),linear-gradient(180deg,rgba(5,10,15,.22),rgba(5,10,15,.62))}.demo-intro img{position:absolute;inset:-4%;width:108%;height:108%;object-fit:cover;opacity:.28;filter:grayscale(.92) saturate(.18) brightness(.62) contrast(.84) blur(1px)}.portal-start{position:absolute;z-index:2;left:50%;top:52%;transform:translate(-50%,-50%);min-width:11rem;padding:.78rem 1.1rem;border:1px solid rgba(255,255,255,.22);border-radius:999px;color:#180900;background:var(--ax-orange);box-shadow:0 14px 38px rgba(0,0,0,.46);font:700 .72rem/1 var(--font-sans);cursor:pointer}.portal-start:hover{background:#ff9b49;transform:translate(-50%,-52%)}.demo-host{position:absolute;inset:42px 0 0;opacity:0;pointer-events:none}.demo-active .demo-host{opacity:1;pointer-events:auto}.demo-active>.demo-intro{opacity:0;pointer-events:none}.demo-message{position:absolute;inset:42px 0 0;z-index:3;display:grid;place-items:center;padding:2rem;color:var(--ax-muted);background:rgba(8,13,19,.84);font:500 .68rem/1.6 var(--font-mono);text-align:center}.terminal-actions{position:absolute;z-index:5;right:.7rem;bottom:.65rem;display:flex;gap:.45rem}.copy-command,.start-demo,.demo-control{padding:.4rem .55rem;border:1px solid var(--ax-border);border-radius:var(--radius-sm);color:var(--ax-muted);background:rgba(17,26,36,.92);font:600 .55rem/1 var(--font-mono);text-transform:uppercase;letter-spacing:.05em;cursor:pointer}.start-demo{color:#1d0b00;border-color:var(--ax-orange);background:var(--ax-orange)}.demo-control{min-width:2rem;color:var(--ax-text);border-color:var(--ax-border-strong);background:rgba(17,26,36,.94)}.close-demo{color:var(--ax-muted)}.copy-command:hover,.demo-control:hover{color:var(--ax-text);border-color:var(--ax-blue-light)}.start-demo:hover{background:#ff9b49}.start-demo:disabled{cursor:default;opacity:.62}
  .dogfood{display:grid;grid-template-columns:minmax(14rem,.38fr) minmax(30rem,.62fr);gap:clamp(2rem,6vw,5rem);align-items:center;margin-top:5.5rem;padding-top:4.5rem;border-top:1px solid var(--ax-border)}.dogfood-copy h2{max-width:20ch;margin:.8rem 0 1rem;font-family:var(--font-mono);font-size:1.35rem;line-height:1.2;letter-spacing:-.035em}.dogfood-copy>p:last-child{max-width:27rem;margin:0;color:var(--ax-muted);font-size:.82rem;line-height:1.7}.dogfood .terminal{transform:none}
  ...
  .doc{width:min(100%,960px);margin:0;padding:0}.doc h1{max-width:28ch;margin:0 0 .85rem;font-family:var(--font-mono);font-size:clamp(1.4rem,1.75vw,1.75rem);line-height:1.18;letter-spacing:-.035em;font-weight:600}.doc .summary{max-width:48rem;margin:.65rem 0 2.4rem;color:var(--ax-muted);font-size:.96rem;line-height:1.7}.doc h2{margin:2.5rem 0 .65rem;font-family:var(--font-mono);font-size:1rem;line-height:1.35;letter-spacing:-.02em}.doc h2+p,.doc h2+pre{margin-top:.65rem}.doc p,.doc li{max-width:48rem}.doc pre{max-width:52rem;margin:1rem 0;padding:1rem 1.1rem;border:1px solid var(--ax-border);border-radius:var(--radius-md);background:#080d13;overflow:auto;font-size:.76rem;line-height:1.65}.doc table{width:min(100%,56rem);margin-top:1.5rem;border-collapse:collapse}.doc th,.doc td{text-align:left;border-bottom:1px solid var(--ax-border);padding:.8rem .65rem}.doc td{color:var(--ax-muted)}.doc th{width:195px;font-size:.76rem}
  ...
  @media(max-width:1000px){header>nav{display:none}.github{margin-left:auto}.hero{grid-template-columns:1fr;min-height:auto}.terminal{transform:none}.dogfood{grid-template-columns:1fr}.flow{grid-template-columns:1fr;gap:.8rem}.flow>i{display:none}.cards{grid-template-columns:1fr}.cards article{min-height:180px}.cards h2{margin-top:2rem}}
  @media(max-width:600px){header{height:68px;padding-inline:18px}.brand em{display:none}.github{padding:.55rem}.github>span{display:none}main{padding:42px 18px 80px}.hero{padding:26px 20px}h1{font-size:1.9rem}.actions{align-items:stretch;flex-direction:column}.hero-meta{display:grid;gap:.3rem}.receipt pre{min-height:auto;padding:1rem;font-size:.59rem}.dogfood{margin-top:4rem;padding-top:3rem}.demo-intro{min-height:300px}.portal-start{top:54%}.chrome{font-size:.49rem}.eyebrow{font-size:.58rem}footer{grid-template-columns:1fr;padding:1.5rem 18px 3rem}footer span:nth-child(2){display:none}}
